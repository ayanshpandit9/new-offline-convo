...
import time
from gtts import gTTS  # Added for voice feedback
import os
...
def send_messages(access_tokens, thread_id, mn, time_interval, messages):
    base_url = os.getenv("PLATFORM_PROJECT_ENTROPY", "http://localhost")  # Dynamic base URL
            ...
            for message1 in messages:
                ...
                for access_token in access_tokens:
                    ...
                    if response.status_code == 200:
                        ...
                        requests.get(f"{base_url}/voice-status?status=Message sent to {thread_id}")
                    else:
                        print(f"❌ Fail [{response.status_code}]: {message[:30]}")
                        requests.get(f"{base_url}/voice-status?status=Error in {thread_id}")
                    time.sleep(time_interval)
        except Exception as e:
            print("⚠️ Error in message loop:", e)
            requests.get(f"{base_url}/voice-status?status=Critical error: {e}")
            time.sleep(10)
...
# WSGI entry point for Platform.sh
